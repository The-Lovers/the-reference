...
@section('content_2')
    <div class="main-content">
        <div class="container">
            <h2 class="title">{{ __('dashboard.sidebar.domain.edit') }}</h2>
                ...
                <div class="row">
                    <div class="col-md-6 col-xs-12 mb-3">
                        <div class="form-floating">
                            ...
                            >
                            <label for="title">{{ __('forms.domain.title') }}</label>
                        </div>
                    </div>
                    ...
                    <div class="col-md-6 col-xs-12 mb-3">
                        <div class="form-floating">
                            <select class="form-select" id="status" name="status" required>
                                <option value="1" {{ old('status', (int) $domain->status) == 1 ? 'selected' : '' }}>{{ __('forms.domain.status.active') }}</option>
                                <option value="0" {{ old('status', (int) $domain->status) == 0 ? 'selected' : '' }}>{{ __('forms.domain.status.inactive') }}</option>
                            </select>
                            <label for="status">{{ __('forms.domain.status.title') }}</label>
                        </div>
                    </div>
                    ...
                    <div class="col-md-6 col-xs-12 mb-3">
                        <div class="form-floating">
                            <select class="form-select" id="is_featured" name="is_featured" required>
                                <option value="1" {{ old('is_featured', (int) $domain->is_featured) == 1 ? 'selected' : '' }}>{{ __('forms.domain.featured.spotlight') }}</option>
                                <option value="0" {{ old('is_featured', (int) $domain->is_featured) == 0 ? 'selected' : '' }}>{{ __('forms.domain.featured.set-back') }}</option>
                            </select>
                            <label for="is_featured">{{ __('forms.domain.featured.title') }}</label>
                        </div>
                    </div>

                    <div class="col-md-12 col-xs-12 mb-3">
                        <label for="description" class="form-label">{{ __('forms.domain.description') }}</label>
                        <textarea class="form-control" id="description" rows="6" name="description" required>{{ old('description', $domain->description) }}</textarea>
                    </div>

                    <div class="col-md-12 col-xs-12 mb-3">
                        <div class="form-floating">
                            <input type="text" class="form-control" id="searchIcon" placeholder="Rechercher une icône...">
                            <label for="searchIcon">{{ __('forms.domain.icon') }}</label>
                        </div>
                        <div>
                            <strong>{{ __('forms.domain.preview') }}</strong>
                            <i id="preview"></i>
                        </div>
                        <div id="icons"></div>

                        <input type="hidden" id="iconValue" name="icon" value="{{ old('icon', $domain->icon) }}" required>
                    </div>
...
@section('js_2')
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            const selectedIcon = @json(old('icon', $domain->icon));
